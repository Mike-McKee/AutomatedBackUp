...
TO DO:
------
1. Make directories.SOURCE a list of paths instead of a string
2. Make main() iterate through each item in SOURCE and run recursive list_files_in_directory() on each
3. Add "none" option for get_last_modifiedtime() time_threshold parameter so we can test without
4. Create Logger to log into a .txt file every time the function runs
5. Big change... We got the script to iterate through each source path, but now we have to copy over the directories
"""
